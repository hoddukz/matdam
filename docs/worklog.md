...
### 시장분석 프레임워크 정정 (demand-monitoring.md)

**D축(경쟁사) Maangchi 포럼 관련 오류 정정:**

- ~~"Maangchi 포럼 공식 폐쇄(스팸 원인)" — 최근 이벤트로 기록~~ → Maangchi 포럼은 2015년경 이미 제거 완료 (talk 페이지 "May 2015 letter" 참조). "최근 폐쇄"는 오판
- ~~"Maangchi 쇠퇴 가속 = 기회 확대"~~ → 직접 경쟁 제한적이나, 대체재 집합(유튜브/틱톡/AI 레시피 앱)이 주 위협
- 2/17, 2/19, 3/3 기록에 취소선 + 정정 내용 반영

**프레임워크 개선 방향 (외부 LLM 크로스체크 결과):**

- A축: 절대 임계치(upvote 50/댓글 20) → 구독자 대비 정규화 반응률로 변경 필요
- B축: 키워드 20개 고정 + 주간 카운트 + "해결 여부" 추적 루틴화
- C축: TikTok/IG 해시태그 수치는 핵심 근거에서 제외, 트렌드 방향 확인용으로만 사용
- D축: "AI 레시피 앱" 카테고리 추가 모니터링
...
- [x] 2026-07-12 — 마이그레이션 015·016 프로덕션 적용 완료 (psql Session pooler 경유. 016은 사전 점검 결과 대상 0건 → no-op 확인. 대시보드 SQL Editor는 문장 분리/세션 미유지 문제로 긴 마이그레이션에 사용 불가)
- [x] 2026-07-05 — AI 번역 파이프라인 버그 수정 + 보안 패치 (next 15.5.20, next-intl 4.13.1) + 유지보수 점검
